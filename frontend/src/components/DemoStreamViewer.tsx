...
interface DemoStreamViewerProps {
  events: StreamEvent[];
  isStreaming: boolean;
}

export function DemoStreamViewer({ events, isStreaming }: DemoStreamViewerProps) {
  ...
  useEffect(() => {
    events.forEach((ev: StreamEvent) => {
      const agent = ev.agent ?? (ev.data && typeof ev.data === 'object' && 'agent' in ev.data ? (ev.data as { agent?: string }).agent : undefined);
      if (ev.type === 'agent_start' && agent) {
        setAgentStatuses((prev) => ({ ...prev, [agent]: 'running' }));
      } else if (ev.type === 'agent_complete' && agent) {
        setAgentStatuses((prev) => ({ ...prev, [agent]: 'complete' }));
      } else if (ev.type === 'connected' && ev.data?.agents) {
        // Initial connected event may list agents
        const agents = (ev.data as { agents?: { id: string }[] }).agents || [];
        agents.forEach((a: { id: string }) => {
          if (!agentStatuses[a.id]) setAgentStatuses((prev) => ({ ...prev, [a.id]: 'pending' }));
        });
      }
    });
  }, [events]);
          ...
          {agents.map((agent) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant={
                    agent.status === 'complete' ? 'default' : agent.status === 'running' ? 'secondary' : 'outline'
                  }
                >
                  {agent.status === 'complete' ? 'Complete' : agent.status === 'running' ? 'Running' : 'Pending'}
                </Badge>
              </div>
            </div>
          ))}
        ...
        {events.length > 0 && (
          <div className="mt-4 p-3 bg-gray-800 rounded-lg max-h-48 overflow-y-auto">
            ...
            <div className="space-y-1 font-mono text-xs">
              {events.slice(-8).map((ev: StreamEvent, idx: number) => (
                <div key={idx} className="text-gray-300">
                  {ev.type}{ev.agent ? `: ${ev.agent}` : ''}
                </div>
              ))}
            </div>
          </div>
